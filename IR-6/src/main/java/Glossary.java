...
public class Glossary {
    ...
    private void processInput() {
        try {
            String path = "D:/JavaPro/Library/gutenberg/1";
//            streamContent(path);
            mergeParts();
        } catch (IOException e) {
...
//        System.out.println("Usage: " + percent);
        if (percent < 30) System.out.println("New stream due to low memory remaining");
        if (tokenized.size() >= THRESHOLD || percent < 30) {
    ...
    private void psimi(ArrayList<File> files) throws IOException {
        ...
        for (File file : files) {
            ...
            if (booksID % 100 == 0 || booksID % THRESHOLD == 0) printRuntime(map);
            booksID++;
        }
        printMap(map);
    }
